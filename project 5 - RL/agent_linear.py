...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    if np.random.random(1) < epsilon:
        ...
        ind = np.unravel_index(np.argmax(theta @ state_vector, axis=None), (NUM_ACTIONS, NUM_OBJECTS))
        action_index, object_index = ind
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    if not terminal:
        ...
        max_q = np.max(theta @ next_state_vector, axis=None)
        # CHECK theta[tuple2index(action_index,object_index)] = (1-ALPHA)*(theta @ current_state_vector)[tuple2index(action_index,object_index)] + (ALPHA *(reward + GAMMA * max_q))*current_state_vector
        theta[tuple2index(action_index, object_index)] += ALPHA*(reward + GAMMA*max_q - (theta @ current_state_vector)[tuple2index(action_index, object_index)])* current_state_vector
    else:
        #q_func[current_state_1, current_state_2, action_index,
        #   object_index] = (1-ALPHA)*q_func[current_state_1, current_state_2, action_index, object_index] + ALPHA*(reward)
        # max_q = np.max(theta @ next_state_vector, axis=None)
        # CHECK theta[tuple2index(action_index,object_index)] = ((1-ALPHA)*(theta @ current_state_vector)[tuple2index(action_index,object_index)] + ALPHA *(reward ))*current_state_vector
        theta[tuple2index(action_index, object_index)] += ALPHA*(reward - (theta @ current_state_vector)[tuple2index(action_index, object_index)])* current_state_vector
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        if not for_training:
            # update reward
            epi_reward = epi_reward +  GAMMA**index*reward
...
if __name__ == '__main__':
    ...
    print(np.sum(epoch_rewards_test, axis=0)/5)
