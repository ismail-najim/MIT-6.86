...
def compute_probabilities(X, theta, temp_parameter):
    ...
    """
    #YOUR CODE HERE
    I = np.matmul(theta,np.transpose(X))/temp_parameter
    M = np.amax(I, axis = 0)
    E = np.exp(I-M)
    D = E.sum(axis=0, dtype = 'float')
    R = E/D
    return R
...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    """
    n = X.shape[0]
    regul = lambda_factor/2*np.sum(np.square(theta))
    R = compute_probabilities(X,theta,temp_parameter)
    loss = 0
    for i in range(n):
        loss = loss - np.log(R[Y[i],i])
...
def compute_test_error_mod3(X, Y, theta, temp_parameter):
    ...
    return np.sum(Y_predicted_mod3 != Y)/len(Y)
